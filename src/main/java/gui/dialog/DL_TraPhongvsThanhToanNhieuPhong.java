                ...
                for (ChiTietHoaDonPhong cthdp : dsCTHDP2) {
                    if (cthdp.getGia() == 0) {
                        ...
                        if (cthdp.getHoaDon().getMaHD().equals(hoaDon.getMaHD()) && cthdp.getPhongHat().getMaPhong().equals(addph.getMaPhong())) {
                            ...
                            PhongHat addph1 = ph_dao.getPhongHatByMaPhong(cthdp.getPhongHat().getMaPhong());
                            System.out.println("5");
                            if (addph1.getLoaiPhong().getMaLoaiPhong().equals("LP001")) {
                                gia = 100000f;
                                thanhTien = gia;
                            } else {
                                gia = 60000f;
...
//                                    i++, addph.getTenPhong(), displayTime, cthdp.getGia()
                                    i++, addph1.getTenPhong(), displayTime, thanhTien
                            });
...
//        }
    ...
    private void btnThanhToanActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
            ...
            @Override
            protected void done() {
                // Close the form on the Event Dispatch Thread (EDT)
                SwingUtilities.invokeLater(() -> {
                    Window window = SwingUtilities.getWindowAncestor(btnThanhToan);
                    if (window != null) {
                        window.dispose();
                    }
                });
