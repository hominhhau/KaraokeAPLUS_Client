...
//        }
    ...
    private void btnNhanPhongActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNhanPhongActionPerformed
        SwingWorker<Void, Void> nhanPhongWorker = new SwingWorker<Void, Void>() {
            @Override
            protected Void doInBackground() throws Exception {
                ...
                LocalDateTime ngayNhanPhong = dsDDP.get(0).getNgayNhan();

                // Kiểm tra ngày có bằng ngày hiện tại không
                if (!now.toLocalDate().isEqual(ngayNhanPhong.toLocalDate())) {
                    JOptionPane.showMessageDialog(null, "Chỉ được nhận phòng trong ngày");
                }
                // Kiểm tra giờ nhận phòng phải sau giờ hiện tại hoặc sau tối đa 1 giờ so với giờ hiện tại
                else if (now.isBefore(ngayNhanPhong) || now.minusHours(1).isAfter(ngayNhanPhong)) {
                    ph_dao = new PhongHatClient();
                    if (ph_dao.getPhongHatByMaPhong(maPhong).getTinhTrangPhong().equals("Trong")) {
                        ...
                        System.out.println("Hoa don da duoc tao");
                        ...
                        System.out.println("Chi tiet hoa don phong da duoc tao");

                        // Cập nhật lại tình trạng phòng

                        // Xóa đơn đặt phòng
//                        boolean xoaThanhCong = ddp_dao.deleteDonDatPhong(maDDP, maPhong);
//                        if (xoaThanhCong) {
//                            System.out.println("Don dat phong da duoc xoa");
//                        } else {
//                            System.out.println("Khong the xoa don dat phong");
//                        }
                        ...
                        JOptionPane.showMessageDialog(null, "Phòng đang được sử dụng chưa thể nhận phòng");
                    }
                } else {
                    JOptionPane.showMessageDialog(null, "Đã quá giờ nhận phòng");
                }
