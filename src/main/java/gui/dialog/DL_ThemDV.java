...
//    }
    ...
    public void loadDVThemTable() {
        ...
        if (hd != null && hd.getHoaDon() != null) {
            String maHD = hd.getHoaDon().getMaHD();
            cthddv_dao = new ChiTietHoaDonDVClient();

            ArrayList<ChiTietHoaDonDV> ds = cthddv_dao.getAllTheMaHDDVforRoomArray(maHD, frmPH.getRoomSelected());
            for (ChiTietHoaDonDV cthddv : ds) {
                if (cthddv.getHoaDon() != null && cthddv.getHoaDon().getMaHD().equals(maHD)) {
                    mh_dao = new MatHangClient();
                    MatHang mh = mh_dao.findMatHang(cthddv.getMatHang().getMaMH());
                    dtmDVThem.addRow(new Object[]{mh.getMaMH(), mh.getTenMH(), cthddv.getSoLuong(), cthddv.getGia()});
                }
            ...
            // Handle the case where hd or hd.getHoaDon() is null
            System.out.println("Error: Unable to load DVThemTable. hd or hd.getHoaDon() is null.");
    ...
    private void btnXongActionPerformed(java.awt.event.ActionEvent evt) {
