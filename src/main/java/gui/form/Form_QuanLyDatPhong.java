...
public final class Form_QuanLyDatPhong extends javax.swing.JPanel {
    ...
    private DonDatPhongInter ddp_dao = new DonDatPhongClient();
    ...
    public void phongTrong() {
        ...
        // Kiểm tra và khởi tạo phongTrongList nếu nó là null
        if (phongTrongList == null) {
            phongTrongList = new ArrayList<>();
        }

        // Kiểm tra danh sách đặt phòng
        List<DonDatPhong> dsDDP = ddp_dao.getalltbDonDatPhong();
        if (dsDDP == null) {
            dsDDP = new ArrayList<>();
        }
        ...
        // Xóa các phòng cần loại bỏ khỏi danh sách phòng trống
        ...
        for (PhongHat phong : phongTrongList) {
            ...
            String maLoaiPhong = (loaiPhong != null) ? loaiPhong.getMaLoaiPhong() : null;
            if ("LP002".equals(maLoaiPhong)) {
...
//    }
    ...
    public void phongDangSuDung() {
        ...
        if (phongTrongList != null) {
            ...
            // Xử lý khi danh sách phòng trống là null
            // Ví dụ: Hiển thị thông báo cho người dùng
            System.out.println("Danh sách phòng trống là null");
        }
    }
...
//        pnlPhongCho.repaint();
    ...
    public void phongCho() {
        ...
        // Set thoiGianBatDau thành ngày hiện tại
        thoiGianBatDau.getDatePicker().setDate(ngayDat);

        // Lấy danh sách đơn đặt phòng theo ngày nhận phòng
        ArrayList<DonDatPhong> dsDonDatPhong = ddp_dao.getDonDatPhongTheoNgayNhanPhong(ngayDat);

        // So sánh phongList và dsDonDatPhong để lấy thông tin các phòng đã đặt
        for (PhongHat phong : phongList) {
            for (DonDatPhong donDatPhong : dsDonDatPhong) {
                if (donDatPhong.getPhongHat() != null && phong.getMaPhong().equals(donDatPhong.getPhongHat().getMaPhong())) {
                    String loaiPhong = phong.getLoaiPhong().getMaLoaiPhong();
                    if ("LP002".equals(loaiPhong)) {
                        icon = iconPhongThuong;
                    } else {
