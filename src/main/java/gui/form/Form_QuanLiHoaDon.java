...
public class Form_QuanLiHoaDon extends javax.swing.JPanel {
    ...
    public Form_QuanLiHoaDon() {
        ...
        TableActionEvent_KhuyenMai event = new TableActionEvent_KhuyenMai() {
                    ...
                    if (JOptionPane.showConfirmDialog(null, "Xác nhận sửa khuyến mãi đã chọn?", "Warning", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                        ...
                        Thread editThread = new Thread(() -> {
                            for (int i = 0; i < selectedRows.length; i++) {
                                ...
                                tblKM.setValueAt(phanTram, rowIndex, 4);
            ...
            public void xoa(int xoa) {
                km_dao = new KhuyenMaiClient();
                if (tblKM.getSelectedRowCount() > 0) {
                    if (JOptionPane.showConfirmDialog(null, "Xác nhận xóa khuyến mãi đã chọn?", "Warring", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
                        int[] selectedRows = tblKM.getSelectedRows();
    ...
    public void DocDuLieu() {
        ...
        for (HoaDon hd : list) {
            ...
            dtmhd.addRow(new Object[]{hd.getMaHD(), hd.getNgayLapHD(), maKH, maNV, maKM, hd.getTongTien()});
        }
    }
    ...
    public void writeFileExcell2() throws IOException {
        ...
        for (HoaDon c : list) {
            ...
            if (c.getMaKM() != null) {
                ...
                cellE.setCellValue(""); // hoặc giá trị mặc định khác tùy vào yêu cầu của bạn
            }
